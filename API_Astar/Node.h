...
void Node::Paint(HDC hdc)
{
    ...
    {
        DeleteObject(hBrush);
        hBrush = CreateSolidBrush(RGB(255, 0, 0)); // 빨강
        oldBrush = (HBRUSH)SelectObject(hdc, hBrush);
    }
    break;
    case OpenNode:
    {
        DeleteObject(hBrush);
        hBrush = CreateSolidBrush(RGB(255, 128, 0)); // 주황
        oldBrush = (HBRUSH)SelectObject(hdc, hBrush);
    }
    break;
    case CloseNode:
    {
        DeleteObject(hBrush);
        hBrush = CreateSolidBrush(RGB(128, 0, 0)); // 짙은 빨강
        oldBrush = (HBRUSH)SelectObject(hdc, hBrush);
    }
    break;
    case Path:
    {
        DeleteObject(hBrush);
        hBrush = CreateSolidBrush(RGB(128, 0, 255)); // 보라
        oldBrush = (HBRUSH)SelectObject(hdc, hBrush);
    }
    break;
    default:
